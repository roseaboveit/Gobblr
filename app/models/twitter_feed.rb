class TwitterFeed < Feed
  ...
  def self.set_home_tweets(token,secret,username)
    ...
    @twitter_user.home_timeline.each do |post|
      if @feed.posts.where(url: post[:url].to_s).count == 1
        ...
        @post = Post.create(author: post.user.screen_name, aurl: post.user.profile_image_url.to_s, published: post['created_at'], content: post.text, content_type: "text", url: post[:url].to_s, feed_id: @feed.id, post_id: post.id)
      end
    end
  ...
  def self.set_posts(twitter_identifier, id)
    ...
    bunch_of_posts.each do |post|
      @post = Post.create(author: post.user.screen_name, aurl: post.user.profile_image_url.to_s, published: post['created_at'], feed_id: id, content: post.text, content_type: "text", url: post[:url].to_s, post_id: post.id)
    end
  rescue Twitter::Error::TooManyRequests
  end
end
